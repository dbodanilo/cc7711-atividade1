...
static double offsets[2] = {0.8, 0.15};
...
bool robot_init(Robot *r)
{
  ...
  r->velocities[LEFT] = offsets[0] * MAX_SPEED;
  r->velocities[RIGHT] = offsets[0] * MAX_SPEED;
                       ...
                       int rx, int ry)
{
  ...
  for (int i = -1; i < 2; i += 1)
  {
    for (int j = -1; j < 2; j += 1)
    {
      ...
      if ((xi >= 0 && xi < MAP_LENGTH) && (yj >= 0 && yj < MAP_LENGTH))
      {
...
Box *get_target(Robot *r, Box *bs)
{
  ...
  double min_dist = get_distance(-1, -1, 1, 1);
...
//    printf("limit: %.2lf\n", limit);
    ...
    if (target == NULL || target->visited)
      target = get_target(&epuck, boxes);

    if (target != old_target)
      map_target(arena, &epuck, boxes, target);
    ...
    if (rx != old_x || ry != old_y)
    {
      ...
      // aim for the center of the next square
      double dx = x - rx, dy = y - ry;
//      double hip = get_distance(0.0, 0.0, dx, dy);

      printf("vector: %.2lf, %.2lf\n", dx, dy);
      ...
      // x positivo
      if (dx > 0) {
        if (dy > 0) angle = -3.0 * PI / 4.0;
        else if (dy < 0) angle = -PI / 4.0;
        // dy == 0
        else angle = -PI / 2.0;
      } else if (dx < 0) {
        if (dy > 0) angle = 3.0 * PI / 4.0;
      ...
      // x = 0
      else if (dy > 0) angle = PI;
      else if (dy < 0) angle = 0.0;
      else angle = -PI;

      // -3.0 * PI / 4.0: x e y positivos
      // -PI / 2.0: x positivo
      // -PI/4.0: x positivo, y negativo      
      
      // PI/4.0: x e y negativos
      // PI/2.0: x negativo      
      // 3.0 * PI/4.0: x negativo, y positivo
    ...
    epuck.velocities[LEFT] = offsets[0] * MAX_SPEED;
    epuck.velocities[RIGHT] = offsets[0] * MAX_SPEED;

    double v_dev[] = {0.0, 0.0};
    double sum = 0.0;
    ...
    for (int i = 0; i < 2; i++)
    {
      ...
      for (int j = 0; j < NUM_SENSORS; j++)
      {
        ...
        v_dev[i] += epuck.sensors[j].value * weights[j][i] / 512.0;
      }

      sum += fabs(v_dev[i]);
    }

//    printf("deviation velocity (%%):\n");
    for (int i = 0; i < 2; i += 1) {
      v_dev[i]  /= sum;
...
//             100.0 * v_dev[i]);
      ...
      epuck.velocities[i] += v_dev[i] * offsets[1] * MAX_SPEED;
